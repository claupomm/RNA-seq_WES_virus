...
#SBATCH -p short            # p=ultrashort <2h, p=short <6h, p=mid <2d, p=long <4d
...
$star --outFileNamePrefix ../star_wes/$sample.join. --genomeDir $genome --runThreadN 14 --readFilesIn ../trim/$sample.fq.join.gz --outStd BAM_SortedByCoordinate --chimSegmentMin 20 --outSAMtype BAM SortedByCoordinate --outSAMmode Full --outFilterMismatchNmax 10 --readFilesCommand zcat --twopassMode Basic > $sample.join.bam
echo "Done."
date
echo "Alignment via STAR, hg38+viruses hybrid genome, unmerged seqs..."
$star --outFileNamePrefix ../star_wes/$sample.un. --genomeDir $genome --runThreadN 14 --readFilesIn ../trim/$sample.fq.un*.gz --outStd BAM_SortedByCoordinate --chimSegmentMin 20 --outSAMtype BAM SortedByCoordinate --outSAMmode Full --outFilterMismatchNmax 10 --readFilesCommand zcat --twopassMode Basic > $sample.un.bam
echo "Done."
date
...
rm ../trim/$sample.*gz
rm -r ../star_wes/$sample.*._STARtmp ../star_wes/$sample.*.Log.out ../star_wes/$sample.*._STARgenome ../star_wes/$sample.*._STARpass1
date

echo "Save chimeric reads + fasta files..."
mv ../star_wes/$sample.join.Chimeric.out.sam .
mv ../star_wes/$sample.un.Chimeric.out.sam .
